...
export function CourtScene() {
  ...
  }, [activeSession?.turns.length, isLoading]);
  ...
  return (
    <div className="court-scene">
      <div className="court-scene__header">
        <span className="court-scene__title">升堂问案</span>
        ...
        <span className={`court-scene__turns-left ${turnsRemaining <= 1 ? 'court-scene__turns-left--low' : ''}`}>
      ...
      <div className="court-scene__turns">
        ...
        {activeSession.turns.map((turn, i) => {
          ...
          const citedEntryForTurn = turn.citedEntryId
            ...
            : undefined;
          // NPC 交换检测：当前是 NPC，前一条也是 NPC（不同角色）
      ...
      {isForcedConclusion ? (
        ...
        <div className="court-input">
          ...
          <div className="court-input__row">
            <button
              className="court-input__evidence-btn"
              onClick={() => setShowEvidenceSelector(s => !s)}
            >
              引用
            </button>
            <textarea
      ...
      {showEvidenceSelector && (
        <div className="court-evidence-overlay" onClick={() => setShowEvidenceSelector(false)}>
          <div className="court-evidence-sheet" onClick={e => e.stopPropagation()}>
            <div className="court-evidence-sheet__handle" />
            <div className="court-evidence-sheet__title">选择引用案卷记录</div>
            <div className="court-evidence-sheet__list">
              {allEntries.length === 0 ? (
                <div className="court-evidence-sheet__empty">暂无案卷记录可引用</div>
              ) : (
                allEntries.map(entry => {
                  ...
                  return (
                    <button
                      key={entry.id}
                      className={`court-evidence-option ${citedEntryId === entry.id ? 'court-evidence-option--selected' : ''}`}
                      onClick={() => { setCitedEntryId(entry.id); setShowEvidenceSelector(false); }}
                    >
                      <span className="court-evidence-option__speaker">[{speakerLabel}]</span>
                      <span className="court-evidence-option__text">{entry.rawDialogueSummary}</span>
                    </button>
                  );
                })
              )}
            </div>
            <button className="court-evidence-sheet__cancel" onClick={() => setShowEvidenceSelector(false)}>
              取消
            </button>
          </div>
        </div>
